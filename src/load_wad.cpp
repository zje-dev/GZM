...
void load_wad (string name) {
	...
	if (wad.is_open()) {
		...
		std::experimental::filesystem::create_directories((read_project("file.path") + read_project("project.name") + "/" + read_project("project.name")).c_str());
		std::experimental::filesystem::create_directories((read_project("file.path") + read_project("project.name") + "/" + read_project("project.name") + "/sprites").c_str());
		for (auto it = all_lumps.begin() ; it != all_lumps.end(); it++) {
			if (it->name == "DECORATE") {
				string path = read_project("file.path") + read_project("project.name") + "/" + read_project("project.name");
				...
				string path = read_project("file.path") + read_project("project.name") + "/" + read_project("project.name") + "/sprites";
