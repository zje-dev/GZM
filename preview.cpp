...
static int update (void *data) {
	...
	gtk_image_set_from_file(GTK_IMAGE(gtk_builder_get_object(animation_window, "sprites")),
		(read_project("file.path") + read_project("project.name") + "/sprites/" + sprites->substr(0, 4) + base_animation_window + direction_animation_window + reflect).c_str()
	);
	std::cout << (read_project("file.path") + read_project("project.name") + "/sprites/" + sprites->substr(0, 4) + base_animation_window + direction_animation_window +
		reflect).c_str() << "\n";
	return 0;
}
